...
`useInfiniteQuery`의 옵션은 [`useQuery` 훅](../useQuery)과 동일하지만, 다음 옵션이 추가됩니다:

- `queryFn: (context: QueryFunctionContext) => Promise<TData>`
  - **필수, 단 기본 쿼리 함수가 정의되지 않은 경우에만** [`defaultQueryFn`](../../guides/default-query-function)
  - 쿼리가 데이터를 요청할 때 사용할 함수입니다.
  - [QueryFunctionContext](../../guides/query-functions#queryfunctioncontext)를 받습니다.
...
`useInfiniteQuery`의 반환 속성은 [`useQuery` 훅](../useQuery)과 동일하지만, 다음 속성과 `isRefetching` 및 `isRefetchError`의 작은 차이가 있습니다:
