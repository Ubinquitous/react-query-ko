...
> 참고: 대부분의 애플리케이션에서는 일반적으로 필요하지 않지만, 드문 시나리오(예: 쿼리의 `state.dataUpdatedAt` 타임스탬프를 보고 쿼리가 초기 값으로 사용할 만큼 신선한지 결정할 때)에서 더 많은 정보를 얻는 데 유용할 수 있습니다.
...
> 참고: 대부분의 애플리케이션에서는 일반적으로 필요하지 않지만, 드문 시나리오에서 쿼리에 대한 더 많은 정보를 얻는 데 유용할 수 있습니다.
...
`QueryCache`가 내부적으로 어떻게 작동하는지 더 잘 이해하려면 [#18: Inside React Query](../../framework/react/community/tkdodos-blog#18-inside-react-query)에서 커뮤니티 리소스를 참고하세요.
