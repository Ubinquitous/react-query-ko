...
제공하는 **고유한 키**는 내부적으로 쿼리를 다시 가져오고, 캐시하고, 애플리케이션 전반에 걸쳐 공유하는 데 사용됩니다.
...
- `fetchStatus === 'paused'` - 쿼리가 데이터를 가져오려 했으나 일시 중지되었습니다. 이 부분에 대한 자세한 내용은 [Network Mode](../network-mode) 가이드를 참조하세요.
...
상태 체크를 수행하는 다른 방법에 대해서는 [Community Resources](../../community/tkdodos-blog#4-status-checks-in-react-query)를 참조하세요.
