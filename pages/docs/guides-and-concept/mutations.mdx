...
변수만으로는 뮤테이션이 특별할 게 없지만, `onSuccess` 옵션과 [Query Client의 `invalidateQueries` 메서드](../../../../reference/QueryClient/#queryclientinvalidatequeries) 및 [Query Client의 `setQueryData` 메서드](../../../../reference/QueryClient/#queryclientsetquerydata)와 함께 사용하면, 뮤테이션은 매우 강력한 도구가 됩니다.
...
`useMutation`은 뮤테이션 생애 주기 동안 어느 단계에서나 빠르고 쉽게 사이드 이펙트를 처리할 수 있는 몇 가지 헬퍼 옵션을 제공합니다. 이러한 옵션들은 [뮤테이션 후 쿼리 무효화 및 재패칭](../invalidations-from-mutations)과 [낙관적 업데이트](../optimistic-updates)에도 유용합니다.
...
// 애플리케이션 종료 시 일시 중지된 뮤테이션을 탈수할 수 있습니다:
const state = dehydrate(queryClient);

// 애플리케이션이 시작되면 뮤테이션을 다시 수화할 수 있습니다:
...
오프라인 뮤테이션을 [persistQueryClient 플러그인](../../../react/plugins/persistQueryClient)으로 영속화할 경우, 페이지를 새로 고침했을 때 뮤테이션을 재개할 수 없으므로 기본 뮤테이션 함수를 제공해야 합니다.
...
또한, [오프라인 예제](../../examples/offline)에서는 쿼리와 뮤테이션 모두를 다루는 광범위한 예제를 제공합니다.
...
뮤테이션에 대한 자세한 내용은 [#12: Mastering Mutations in React Query](../../community/tkdodos-blog#12-mastering-mutations-in-react-query)**에서 확인하실 수 있습니다.**
