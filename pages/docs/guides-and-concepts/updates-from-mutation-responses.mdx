...
서버에서 객체를 **업데이트**하는 뮤테이션을 처리할 때, 새 객체가 뮤테이션 응답에서 자동으로 반환되는 것이 일반적입니다. 이미 가지고 있는 데이터에 대해 네트워크 호출을 낭비하는 대신, 뮤테이션 함수가 반환한 객체를 활용하여 기존 쿼리를 새 데이터로 즉시 업데이트할 수 있습니다. 이를 위해 [Query Client의 `setQueryData`](../../../../reference/QueryClient/#queryclientsetquerydata) 메서드를 사용할 수 있습니다:
