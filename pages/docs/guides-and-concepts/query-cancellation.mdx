...
기본적으로, 약속이 해결되기 전에 언마운트되거나 사용되지 않게 된 쿼리는 _취소되지 않습니다_. 즉, 약속이 해결된 후에도 결과 데이터는 캐시에 남아 있습니다. 이는 쿼리를 수신하기 시작한 후, 컴포넌트를 언마운트했더라도 쿼리가 아직 가비지 컬렉션되지 않았다면 데이터가 여전히 사용 가능하다는 점에서 유용합니다.
