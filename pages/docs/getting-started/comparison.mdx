...
> **<sup>4</sup> 사용 전 쿼리 구성** - 이는 query와 mutation이 사용되기 전에 동작 방식을 구성할 수 있다는 뜻입니다. 예를 들어, 쿼리는 사전에 기본값으로 완전히 구성될 수 있으며 사용 시 `useQuery({ queryKey })`만 필요하며, 매번 가져오기 함수 또는 옵션을 전달할 필요가 없습니다. SWR에는 이 기능의 일부 형태가 있으며 기본 가져오기 함수를 사전 구성할 수 있지만, 이는 전역 가져오기 함수로만 가능하며, 개별 쿼리 기반이 아니며 변경에도 적용되지 않습니다.

> **<sup>5</sup> mutation 후 자동 refetching** - mutation이 발생한 후 진정한 자동 다시 가져오기가 이루어지려면, 스키마가 필요하며(예: GraphQL이 제공하는 것) 라이브러리가 해당 스키마에서 개별 엔터티 및 엔터티 유형을 식별하는 방법을 알아야 합니다.
